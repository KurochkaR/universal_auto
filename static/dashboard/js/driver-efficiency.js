		...
		success: function (data) {
			...
			if (data[0]['drivers_efficiency'].length !== 0) {
				...
				data[0]['drivers_efficiency'].forEach(function (driver) {
					...
					driverInfo.append('<p>' + gettext("Виконаних замовлень: ") + driver.total_orders_accepted + '</p>');
					...
					data.forEach(function (item, index) {
						...
						efficiency.forEach(function (items, innerIndex) {
							...
							fleets.forEach(function (fleet, fleetIndex) {
								...
								driverInfo.append('<p>' + gettext("Кількість замовлень: ") + fleet[Object.keys(fleet)[0]].total_orders_accepted + '</p>');
