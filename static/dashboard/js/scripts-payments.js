		...
		success: function (response) {
			...
			for (var i = 0; i < response.length; i++) {
				if ((paymentStatus === 'on_inspection' && response[i].status === 'Перевіряється') ||
					...
					(paymentStatus === 'closed' && response[i].status === 'Виплачений')) {
...
//					var confirmButton = '<button class="apply-btn"><img src="' + smsImg + '" alt="SMS Image"></button>';
					...
					} else if (response[i].status === 'Виплачений') {
						row.find('.edit-btn, .apply-btn').hide();
